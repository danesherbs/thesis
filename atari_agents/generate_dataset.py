...
def make_dataset(num_games, pre_processing=True, split_into_train_test=True):
    ...
    if split_into_train_test:
        if not os.path.exists('./record/train/'):
            ...
            # os.makedirs('./record/train/train/')
        if not os.path.exists('record/test/'):
            os.makedirs('./record/test/')
            # os.makedirs('./record/test/test/')
    ...
    for episode in range(num_games):
        ...
        while not ale.game_over():
            if np.mod(iter, 2) == 0:
                ...
                __save_image(screenshot, split_into_train_test, iter/2)
...
def __pre_process(image_array):
    '''
    Takes numpy array and returns processed PIL image.
...
def __save_image(image, split_into_train_test, iter, extension='.png'):
    # save screenshot in appropriate directory
    if split_into_train_test:
...
if __name__ == '__main__':
    num_games = 1
    make_dataset(num_games, pre_processing=True, split_into_train_test=True)