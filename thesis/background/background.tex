...
\subsubsection{Introduction: A Probabilistic Perspective}
...
\]
which measures how much information is lost when we represent $p_{\vec{\theta}}(\vec{z}|\vec{x})$ with $q_{\vec{\phi}}(\vec{z}|\vec{x})$ (measured in nats) \cite{Burnham2002}. Using the KL divergence, our problem now amounts to the optimisation problem \cite{Blei2016}:
\begin{align}
q_{\vec{\phi}^*}(\vec{z}|\vec{x}) = \arg\min_{q_{\vec{\phi}}(\vec{z}|\vec{x})} D_{KL}(q_{\vec{\phi}}(\vec{z}|\vec{x})||p_{\vec{\theta}}(\vec{z}|\vec{x}))
...
\subsubsection{Learning the Generative Parameters with a Neural Network}
...
The first term is the KL divergence between the probabilistic encoder and the prior. To make our lives simpler, we can choose the prior to be the isotropic multivariate Gaussian:
\begin{align}
p_{\vec{\theta}}(\vec{z}) = \mathcal{N}(\vec{0}, \vec{I})
\end{align}
We will also assume that the posterior is a multivariate Gaussian with diagonal covariance:
\begin{align}
p_{\vec{\theta}}(\vec{z} | \vec{x}^{(i)}) = \mathcal{N}(\vec{\mu}^{(i)}, \vec{\sigma}^{2(i)}\vec{I})
\end{align}
