...
In this section we'll develop the intuition behind the variational autoencoder.

We've been considering the data set $X = \{ \vec{x}^{(i)} \}_{i=1}^{N}$. In this section we'll take $X$ to be the MNIST data set (the vector $\vec{x^{(i)}}$ now corresponds to a flattened MNIST image). 

\begin{figure}[h!]
\centering
\captionsetup{justification=centering}
\includegraphics[scale=0.7]{background/variational_autoencoder_latent_space_colour_0_with_mnist.eps}
\caption{Caption. \cite{Dykeman2016}}
\label{fig:variational_autoencoder_latent_space_colour_0_with_mnist}
\end{figure}
...
\centering
\captionsetup{justification=centering}
\includegraphics[scale=0.45]{background/variational_autoencoder_latent_space_adding_latent_spaces.eps}
...
\label{fig:variational_autoencoder_latent_space_adding_latent_spaces}
\end{figure}
\end{landscape}
