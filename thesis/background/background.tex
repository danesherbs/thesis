...
In this section we'll develop the intuition behind the variational autoencoder. We've been considering the data set $X = \{ \vec{x}^{(i)} \}_{i=1}^{N}$. In this section we'll take $X$ to be the MNIST data set (the vector $\vec{x^{(i)}}$ now corresponds to a flattened MNIST image). For visualisatoin purposes, we'll also take the latent space dimension $k = 2$, that is, $\vec{z} = (z_1, z_2)$.

The probabilistic encoder (\ref{eq:approximate_posterior_gaussian})
\[
q_{\vec{\phi}}(\vec{z} | \vec{x}^{(i)}) = \mathcal{N}(\vec{\mu}^{(i)}, \vec{\sigma}^{2(i)}\vec{I})
\]
describes where the latent variable $\vec{z}$ lies for data point $\vec{x}^{(i)}$. In Figure (\ref{fig:variational_autoencoder_latent_space_colour_0_with_mnist_updated}), two mappings are shown from 
